...
class AnnouncementsSpec {
  ...
  def allTests = Future {
...

    {
      ...
      val (node_a_sig, bitcoin_a_sig) = signChannelAnnouncement(Block.BCARegtestForkBlockHash, 42, node_a, node_b.publicKey, bitcoin_a, bitcoin_b.publicKey, "")
      val (node_b_sig, bitcoin_b_sig) = signChannelAnnouncement(Block.BCARegtestForkBlockHash, 42, node_b, node_a.publicKey, bitcoin_b, bitcoin_a.publicKey, "")
      val ann = makeChannelAnnouncement(Block.BCARegtestForkBlockHash, 42, node_a.publicKey, node_b.publicKey, bitcoin_a.publicKey, bitcoin_b.publicKey, node_a_sig, node_b_sig, bitcoin_a_sig, bitcoin_b_sig)
...

    {
      ...
      val ann = makeChannelUpdate(Block.BCARegtestForkBlockHash, alicePk, randomKey.publicKey, 45561, 10, 10000, 100, 1, true, System.currentTimeMillis / 1000)
...

    {
      ...
      val channelUpdate1 = makeChannelUpdate(Block.BCARegtestForkBlockHash, node1_priv, node2_priv.publicKey, 0, 0, 0, 0, 0, isEnabled = true, System.currentTimeMillis / 1000)
      val channelUpdate1_disabled = makeChannelUpdate(Block.BCARegtestForkBlockHash, node1_priv, node2_priv.publicKey, 0, 0, 0, 0, 0, isEnabled = false, System.currentTimeMillis / 1000)
      val channelUpdate2 = makeChannelUpdate(Block.BCARegtestForkBlockHash, node2_priv, node1_priv.publicKey, 0, 0, 0, 0, 0, isEnabled = true, System.currentTimeMillis / 1000)
      val channelUpdate2_disabled = makeChannelUpdate(Block.BCARegtestForkBlockHash, node2_priv, node1_priv.publicKey, 0, 0, 0, 0, 0, isEnabled = false, System.currentTimeMillis / 1000)
